...
export default function MistakesPage() {
  ...
  const loadQuestion = useCallback(async (nextMode: QuizMode) => {
    ...
    try {
      ...
      const response = await fetch(`/api/review-question?mode=${nextMode}&exclude=${exclude}&only=${only}`, { cache: 'no-store' });
      const data = await response.json();
  ...
  useEffect(() => {
    ...
    void initialize();
  }, [loadMistakes, loadQuestion, mode, resetCycle]);
            ...
            {result ? (
              <section className={result.isCorrect ? 'resultBox correct' : 'resultBox wrong'}>
                ...
                <button className="nextPrimary stickyNext" disabled={isLoading} onClick={handleNext} type="button">
                  {hasAnsweredFinalQuestion ? '結果へ' : '次の復習へ'}
                </button>
              </section>
            ) : null}
          </>
