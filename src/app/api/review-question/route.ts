...
import { getUnresolvedMistakes, getVocabulary } from '@/lib/sheets';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [mistakes, vocabulary] = await Promise.all([getUnresolvedMistakes(), getVocabulary()]);
    const choicePool = [...mistakes, ...vocabulary];
    return NextResponse.json(createMistakeQuestion(mistakes, mode, { excludedItemKeys, onlyItemKeys }, choicePool));
  } catch (error) {
